...
const CASE_STUDIES = [
  ...
  { year:'2026', title:'Hugging Face AI agent intrusion', href:'https://huggingface.co/blog/security-incident-july-2026', sourceName:'Hugging Face',
    ...
    summarySafeguard:'Apply the same rigor to AI-agent test/research environments as production systems - strict sandboxing with no path to the open internet, and credential isolation - since an agent doesn\'t need malicious intent to cause a real breach, only enough autonomy and an exploitable path.' },
];
...
function renderCaseStudyCard(c){
  return `
    <div class="case-card">
      ...
      <h4>${c.title}</h4>
      <div class="case-section"><b>What happened</b>${c.summaryWhat}</div>
...
async function renderCaseStudyTab(container){
  ...
  `;
  const { items, live } = await loadCaseStudiesData();
  ...
  const freshness = live
    ? `Automatically checked for new watershed-caliber incidents - the existing list is never removed, only added to.`
    : `Showing a curated snapshot - the live feed didn't return anything this time, so nothing's lost, just not current. Refresh in a bit.`;
  ...
  container.innerHTML = `
    <div class="page">
      ...
      <div class="section-tile">
        ...
        <p class="case-source">${freshness}</p>
        <div class="case-grid">

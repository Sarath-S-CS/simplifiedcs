...
7. **Escape everything you interpolate into HTML, and keep the CSP intact.** Any text or URL the site didn't author itself (assessment answers, the live Supabase feeds, AI responses) goes through `escapeHtml()` / `safeHttpUrl()` from `src/ui/html-safety.js` before it lands in an `innerHTML` template. The site ships a strict Content-Security-Policy in `_headers` (`scripts/serve.js` applies it locally too): no inline `<script>`, no `onclick=`-style handlers, no `eval`. A new third-party script or API host needs an explicit `_headers` update, or the browser will silently block it.

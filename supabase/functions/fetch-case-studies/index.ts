...
// Cost-runaway guard, distinct in mechanism from fetch-news/fetch-exploits'
// identical-looking guard: those check their own data table's fetched_at,
// which is safe because a real run always writes something. Here the
// expected common outcome is finding nothing and writing zero case_studies
// rows, so this needs its own append-only log (case_study_fetch_log) that
// records every attempt regardless of outcome - otherwise a string of
...
Deno.serve(async (_req) => {
  ...
  if (!apiKey) {
    ...
    return Response.json({ skipped: true, reason: "ANTHROPIC_API_KEY not configured" });
  }
...
async function callClaude(prompt: string, apiKey: string): Promise<Candidate[]> {
  ...
  if (!toolUse) return []; // model ended without calling the report tool - treat as "nothing found"
...
async function checkRateLimit(supabase: ReturnType<typeof createClient>): Promise<boolean> {
  const { data } = await supabase
    .from("case_study_fetch_log")
    .select("attempted_at")
    .order("attempted_at", { ascending: false })
    .limit(1)
    .maybeSingle();
  if (!data) return false;
  const hoursSince = (Date.now() - new Date(data.attempted_at).getTime()) / 3_600_000;
